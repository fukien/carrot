...
void Projection::destroy(){
 ...
 void Projection::initProjection(Table*table,string columnName[] ,int columnNum){
            ...
            if(columnNum==-1){
                ...
                for(int i=0;i<columnNum;i++) this->fieldList[i]=i;
                this->fieldflag=true;
            }else{
                this->fieldList=new int[columnNum];
            }
...
SPJItem* Projection::buildSPJItem(){
        ...
        for(int i=0;i<item->fieldNum;i++) item->data[i]=0;
         ...
         if(this->fieldflag==false){
            ...
            for(int i=0;i<fieldNum;i++){
                   string ss=tuple->column[i].field->fname;
                    if(mp.count(ss)>0){
                            if(item->data[cnt]==0) item->data[cnt]=new Byte[tuple->column[i].field->len];
            ...
            for(int i=0;i<this->columnNum;i++){
                   if(item->data[i]==0) item->data[i]=new Byte[tuple->column[this->fieldList[i]].field->len];
         ...
         if(this->fieldflag==false){
            ...
            for(int i=0;i<item->fieldNum;i++){
                   string ss=sourceItem->fieldName[i];
                    if(mp.count(ss)>0){
                            if(item->data[cnt]==0) item->data[cnt]=new Byte[sourceItem->len[i]];
            ...
            this->fieldflag=true;
             ...
             for(int i=0;i<this->columnNum;i++){
                  if(item->data[i]==0) item->data[i]=new Byte[sourceItem->len[this->fieldList[i]]];
